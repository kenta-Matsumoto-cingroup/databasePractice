...
SELECT surface_area AS アジアの最小面積 FROM countries WHERE continent = 'Asia' ORDER BY surface_area ASC LIMIT 1;
...
SELECT name AS name, language FROM countries JOIN public.country_languages ON code = country_code ORDER BY name, language;

-- 問26
-- 全ての国と言語と市区町村を表示してください。
SELECT countries.name AS 国名, cities.name AS 市町村名, country_languages.language FROM public.countries
JOIN public.cities ON countries.code = cities.country_code JOIN public.country_languages ON countries.code = country_languages.country_code
ORDER BY countries.name, cities.name, country_languages.language;

-- 問27
-- 全ての有名人を出力してください。左側外部結合を使用して国名なし（country_codeがNULL）も表示してください。
SELECT celebrities.name AS name, countries.name AS name FROM celebrities LEFT JOIN countries ON celebrities.country_code = countries.code;

-- 問28
-- 全ての有名人の名前,国名、第一言語を出力してください。
SELECT celebrities.name AS 名前, countries.name AS 国名, country_languages.language AS 第一言語 FROM celebrities
...
SELECT celebrities.name AS name, (SELECT countries.name FROM public.countries WHERE countries.code = celebrities.country_code) AS 国名 FROM public.celebrities;

-- 問30
-- 最年長が50歳以上かつ最年少が30歳以下の国を表示させてください。
SELECT countries.code AS country_code,
(SELECT MAX(celebrities.age) FROM celebrities WHERE celebrities.country_code = countries.code) AS MAX(ce.age),
(SELECT MIN(celebrities.age) FROM celebrities WHERE celebrities.country_code = countries.code) AS MIN(ce.age)
FROM countries
WHERE (SELECT MAX(celebrities.age) FROM celebrities WHERE celebrities.country_code = countries.code) >= 50
AND (SELECT MIN(celebrities.age) FROM public.celebrities WHERE celebrities.country_code = countries.code) <= 30;

-- 問31
-- 1991年生まれと、1981年生まれの有名人が何人いるか調べてください。ただし、日付関数は使用せず、UNION句を使用してください。
SELECT birth_year AS 誕生年, COUNT(*) AS 人数
FROM (SELECT EXTRACT(YEAR FROM birth) AS birth_year FROM public.celebrities WHERE EXTRACT(YEAR FROM birth) = 1991
UNION ALL SELECT EXTRACT(YEAR FROM birth) AS birth_year FROM public.celebrities WHERE EXTRACT(YEAR FROM birth) = 1981) AS years
...
FROM countries
JOIN public.celebrities ON countries.code = celebrities.country_code
GROUP BY countries.name ORDER BY 平均年齢 DESC;